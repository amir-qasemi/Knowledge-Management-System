...
jQuery.fn.motionCaptcha || (function($) {
	...
	$.fn.motionCaptcha = function(options) {
		...
		/**
		 * Act on matched form element: This could be set up to iterate over
		 * multiple elements, but tbh would it ever be useful?
		 */
		return this.each(function() {
			...
			// Set the canvas DOM element's dimensions to match the display
			// width/height (pretty important):
			...
			// Add canvasWidth and canvasHeight values to context, for Ribbon
			// brush:
			...
			var touchStartEvent = function(event) {
				...
				// Prevent jumpy-touch bug on android, no effect on other
				// platforms:
				_holdStill = true;
				...
				if ( !locked && _isDown ) {
					...
					if (_points.length >= 10) {
						...
						if ( $canvas.attr('class').match(result.Name) && result.Score > 0.7 ) {
							...
							// Destroy the Harmony brush (give it time to finish
							// drawing)
							...
							// Call the onSuccess function to handle the rest of
							// the business:
			 ...
			 * 
			 * @todo For 1.0, if no way to obtain coordinates, don't activate
			 *       MotionCAPTCHA.
			 */
			function getPos(event) {
				var x, y;
				
				// Check for mobile first to avoid android jumpy-touch bug (iOS
				// / Android):
				if ( event.touches && event.touches.length > 0 ) {
					...
					// Chrome/Safari give the event offset relative to the
					// target event:
					x = event.offsetX - borderLeftWidth;
					y = event.offsetY - borderTopWidth;
				} else {
					// Otherwise, subtract page click from canvas offset
					// (Firefox uses this):
	...
	$.fn.motionCaptcha.defaults = {
		actionId: '#mc-action',     // The ID of the input containing the form
									// action
		divId: '#mc',               // If you use an ID other than '#mc' for the
									// placeholder, pass it in here
		canvasId: '#mc-canvas',     // The ID of the MotionCAPTCHA canvas
									// element
		submitId: false,            // If your form has multiple submit buttons,
									// give the ID of the main one here
		cssClass: '.mc-active',     // This CSS class is applied to the form,
		...
		// These messages are displayed inside the canvas after a user finishes
		// drawing:
		errorMsg: 'Please try again.',
		successMsg: 'Captcha passed!',
		
		// This message is displayed if the user's browser doesn't support
		// canvas:
		noCanvasMsg: "Your browser doesn't support <canvas> - try Chrome, FF4, Safari or IE9.",
		
		// This could be any HTML string (eg. '<label>Draw this shit
		// yo:</label>'):
		...
		// NB: The default onSuccess callback function enables the submit
		// button, and adds the form action attribute:
	 ...
	 * 
	 * Jacob O. Wobbrock, Ph.D. | wobbrock@u.washington.edu Andrew D. Wilson,
	 * Ph.D. | awilson@microsoft.com Yang Li, Ph.D. | yangli@cs.washington.edu
	 * 
	 ...
	 * 
	 * The original software is distributed under the "New BSD License"
	 * agreement
	 * 
	...
	// Wrapper for Point class (saves mega kb when compressing the template
	// definitions):
	...
	function Resample(points, n) {
		...
		for (i = 1; i < points.length; i++) {
			...
			if ((D + d) >= I) {
				...
				points.splice(i, 0, q); // insert 'q' at position i in points
										// s.t. 'q' will be the next i
				D = 0.0;
			}
			else D += d;
		}
...
jQuery(document).ready(
		function($) {
			$('#mc-form').motionCaptcha(
					{
						shapes : [ 'triangle', 'x', 'rectangle',
								'circle', 'check', 'zigzag', 'arrow',
								'delete', 'pigtail', 'star' ]
					});
		});